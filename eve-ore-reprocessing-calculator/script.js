...
document.getElementById("calculate-button").addEventListener("click", (e) => {
  ...
  lines.forEach(line => {
    ...
    if (!oreMatch) {
      oreMatch = Object.keys(iceData).find(k => rawOre.toLowerCase().endsWith(k.toLowerCase()));
    }
    if (!oreMatch) {
      oreMatch = Object.keys(oreData).find(k => k.toLowerCase() === rawOre.toLowerCase());
    }
    if (!oreMatch) {
      oreMatch = Object.keys(oreData).find(k => rawOre.toLowerCase().endsWith(k.toLowerCase()));
    }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!textarea || !oreContainer) return;
